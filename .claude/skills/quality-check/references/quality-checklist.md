...
- [ ] **context.Context**: context.Contextを使用する場合、最初の引数に配置されているか
...
## 10. プロジェクト固有（Plant Diary）

- [ ] **Goフォーマット**: `gofmt -l .` を実行してフォーマットが必要なファイルがないことを確認する
- [ ] **クロスプラットフォーム互換性**: シェルスクリプトやOS依存コードが複数環境（Linux/macOS等）で動作するか
- [ ] **SQLiteの型**: SQLiteの型システムや集約関数（`MAX()`, `MIN()` 等）の戻り値型に注意（文字列を返す場合あり）
