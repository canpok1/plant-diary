...
3. `base-tools:monologue` を実行してから、`/review` スキルで自己レビュー（コード品質 + ドキュメント整合性チェック）を行う
  - **重要**: `/review` スキルのプロンプトが展開されたら、必ずそのワークフローを最後まで実行すること。ユーザーからの割り込みがあっても、スキルの実行を完了させてから対応すること
...
8. `base-tools:monologue` を実行してから、`/retro` スキルで振り返りを行う
  - **重要**: `/retro` スキルのプロンプトが展開されたら、必ずそのワークフローを最後まで実行すること。ユーザーからの割り込みがあっても、スキルの実行を完了させてから対応すること
