...
- **タスク完了後の手順**
  1. `/retro` を実行してチーム全員で振り返りを行う
     - リーダーが各メンバーに担当タスクの振り返り内容（よかった点・問題点）の報告を依頼する（SendMessageを使用）
     - 各メンバーからの報告を集約してretroスキルを実行する
...
実装後の自己レビューでは、以下の観点を確認してください：

### 必須チェック項目
- **要件充足**: Issueの要件を全て満たしているか
- **コード品質**: 既存のコーディングスタイル、命名規則に従っているか
- **バグリスク**: エッジケース、境界値、null/undefinedの未処理がないか
- **セキュリティ**: インジェクション、XSS等の脆弱性がないか
- **テスト**: 必要なテストが追加され、全て成功しているか

### プロジェクト固有のチェック項目
- **Goフォーマット**: `gofmt -l .` を実行してフォーマットが必要なファイルがないことを確認する
- **クロスプラットフォーム互換性**: シェルスクリプトやOS依存コードが複数環境で動作するか
- **データベース**: SQLiteの型システムや関数の戻り値型に注意
- **外部リソース**: 依存関係のバージョン固定、チェックサム検証
