...
**SKILL.md 内で他のスキルを参照する際は、必ず正式な完全名を使用すること**

省略形を使用すると、レビュアー（CodeRabbit 等）が正式名称と一致しないと判断し、CHANGES_REQUESTED が発生する。
...
```markdown
`commit-push-pr` スキルでPRを作成する  # 省略形（namespaceなし）
```

```markdown
`monologue` スキルで通知する  # スラッシュなし
```
...
- [ ] プラグインスキルは `{namespace}:{skill-name}` 形式で記述している
- [ ] ローカルスキルは `/{name}` のスラッシュコマンド形式で記述している
- [ ] 省略形を使用していない
